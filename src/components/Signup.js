...
import { DepartmentCall, SignUpCall } from "../models/LoginFetch";
...
const Signup = () => {
  ...
  const [departmentsView, setDepartments] = useState('');
  ...
  useEffect(() => {
    ...
    DepartmentCall(function (result) {
          ...
          <form method="POST" onSubmit={(e) => { handleSignup(e) }}>
            ...
            <div class="form-group">
              ...
              <select required value={dept} onChange={(e) => setDept(e.target.value)} class="form-control">
                ...
                {
                  departmentsView.length >0 ?
                  departmentsView.map((row) => {
                    return (
